...
const PARTNERSHIP_TYPES = [
  {
    ...
    contact: "research@theveritaai.com",
  },
  {
    num: "02",
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#fff" }}>
      ...
      <section style={{ padding: "72px 48px", borderBottom: `1px solid ${v.border}` }}>
        <div style={{ fontSize: 11, letterSpacing: 3, textTransform: "uppercase", color: v.blueLight, fontWeight: 600, marginBottom: 16 }}>Partnership Types</div>
        <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 32, fontWeight: 600, color: v.navy, marginBottom: 40, lineHeight: 1.2 }}>Three ways to partner with The Verita.</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {PARTNERSHIP_TYPES.map((p, index) => (
            <div key={p.num} className="two-col-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", background: index % 2 === 0 ? v.bgSoft : "#fff" }}>
              <div style={{ padding: "40px 48px", borderRight: `1px solid ${v.border}` }}>
                <div style={{ fontSize: 10, letterSpacing: 3, textTransform: "uppercase", color: v.blueLight, fontWeight: 600, marginBottom: 14 }}>Partnership {p.num}</div>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 28, fontWeight: 600, color: v.navy, lineHeight: 1.25, marginBottom: 16 }}>{p.title}</div>
                <p style={{ fontSize: 14, color: v.textBody, lineHeight: 1.8 }}>{p.body}</p>
              </div>
              <div style={{ padding: "40px 48px" }}>
                <div style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", color: "#8a9aaa", fontWeight: 600, marginBottom: 16 }}>What partners receive</div>
                ...
                <a href={`mailto:${p.contact}`} style={{ fontSize: 13, color: v.blue, fontWeight: 600, textDecoration: "none" }}>Enquire — {p.contact} →</a>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ padding: "72px 48px", borderBottom: `1px solid ${v.border}`, background: v.bgSoft }}>
        <div className="two-col-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }}>
          <div>
            ...
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 36, fontWeight: 600, color: v.navy, marginBottom: 16, lineHeight: 1.2 }}>Tell us about your organization.</div>
            ...
            {["hello@theveritaai.com", "India — New Delhi & Mumbai", "United States — New York & San Francisco"].map((d) => (
            ...
            {submitted ? (
              ...
              <div style={{ background: "#fff", padding: "32px" }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 11, letterSpacing: 1, textTransform: "uppercase" as const, color: "#8a9aaa", fontWeight: 600, display: "block", marginBottom: 6 }}>Partnership type</label>
      ...
      <section style={{ padding: "72px 48px", borderBottom: `1px solid ${v.border}` }}>
        <div style={{ fontSize: 11, letterSpacing: 3, textTransform: "uppercase", color: v.blueLight, fontWeight: 600, marginBottom: 16 }}>Our Partners</div>
        <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 32, fontWeight: 600, color: v.navy, marginBottom: 16, lineHeight: 1.2 }}>Founding partners coming soon.</div>
        ...
        <div className="three-col-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 2, background: v.bgRule }}>
          {["University Partners", "Enterprise Partners", "Policy Partners"].map((type) => (
            <div key={type} style={{ background: "#fff", padding: "32px 28px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 120 }}>
              ...
              <div style={{ fontSize: 12, color: "#9aaabb", fontWeight: 600, letterSpacing: 0.5, textAlign: "center" as const }}>{type}</div>
