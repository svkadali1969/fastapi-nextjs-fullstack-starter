  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={onClose}>
      <div style={{ background: "#fff", maxWidth: 620, width: "100%", padding: "40px", position: "relative", maxHeight: "90vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase" as const, color: v.blueLight, fontWeight: 500, marginBottom: 12 }}>
          {program.audience}
        </div>
        <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 28, fontWeight: 600, color: v.navy, lineHeight: 1.3, marginBottom: 8 }}>
          {program.name}
        </div>

        <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" as const }}>
          {program.duration && <span style={{ fontSize: 12, color: "#6a7a8a" }}>⏱ {program.duration}</span>}
          {program.format && <span style={{ fontSize: 12, color: "#6a7a8a" }}>📍 {program.format}</span>}
        </div>

        {program.homepage_body && (
          <p style={{ fontSize: 14, color: v.textBody, lineHeight: 1.8, marginBottom: 24 }}>{program.homepage_body}</p>
        )}

        {/* Modules */}
        {program.modules.length > 0 && (
          <>
            <div style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase" as const, color: "#8a9aaa", fontWeight: 500, marginBottom: 12 }}>Program modules</div>
        ...
        {program.homepage_differentiator && (
          <div style={{ fontSize: 13, color: "#6a7a8a", fontStyle: "italic", padding: "12px 16px", background: v.bgSoft, borderLeft: `3px solid ${v.border}`, marginBottom: 20 }}>
            {program.homepage_differentiator}
          </div>
        )}
        ...
        <div style={{ borderTop: `1px solid ${v.border}`, paddingTop: 20, display: "flex", gap: 12, flexWrap: "wrap" as const }}>
          ...
          <Link href={`/education#${program.section_slug}`} onClick={onClose} style={{ background: "transparent", color: v.navy, padding: "12px 24px", fontSize: 13, textDecoration: "none", fontWeight: 500, border: `1.5px solid ${v.navy}`, display: "inline-block" }}>
